...
export class RecipeServiceService {
  ...
  createdRecipe(recipe:any):Observable<any>{
    const headers=this.getHeaders();
  ...
  updatedRecipes(recipe:any):Observable<any>{
    const headers=this.getHeaders();
...
likeRecipes(id:any):Observable<any>{
  const headers=this.getHeaders();
  return this.http.put(`${this.baseUrl}/api/recipes/${id}/like`, {
    headers})
    .pipe(
  tap((updatedRecipe:any)=>{
    const currentState = this.recipeSubject.value;
...
deletedRecipes(id:any):Observable<any>{
  const headers=this.getHeaders();
  return this.http
